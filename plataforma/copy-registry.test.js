...
assert.equal(Copy.placement("introTitle", "desconocida"), "Vamos a encontrar un buen comienzo");
assert.equal(Copy.onboarding("p12").steps[2].title, "Tu mundo crecerá contigo");
assert.match(Copy.onboarding("p56").steps[2].body, /ni demasiado fáciles ni demasiado difíciles/);
assert.equal(Copy.onboarding("eso").finish, "Find my starting point");
assert.equal(Copy.onboarding("desconocida").steps[0].visual, "avatar");
