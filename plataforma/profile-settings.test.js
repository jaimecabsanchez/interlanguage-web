...
assert.equal(settings.avatarHair, "original");
...
assert.equal(settings.avatarHairLength, 2);
assert.equal(settings.avatarEyeSpacing, 2);
assert.equal(settings.avatarBrowThickness, 2);
assert.equal(settings.avatarNoseWidth, 2);
assert.equal(settings.avatarMouthCurve, 2);
assert.equal(settings.avatarOutfitColor, "green");
...
assigned = S.save("lucia", { avatarCustomised:true, avatarFaceLength:4, avatarHair:"original" });
assigned = S.setActive("lucia", "female");
assert.equal(assigned.avatarCustomised, true, "volver a cargar el mismo sexo conserva la personalización");
assert.equal(assigned.avatarFaceLength, 4, "conserva los parámetros graduales del alumno");
assigned = S.setActive("lucia", "male");
assert.equal(assigned.avatarCustomised, false, "un cambio de sexo administrativo restaura la nueva lámina base");
...
assert.equal(clean.avatarHair, "original");
